...
// The code creates a set of buttons for a quiz.   
function createButtons() {
  // Create a variable called `qNo` and initialize it to 1.
  var qNo = 1;

  // Loop through the `quizQ.answers` array.
  for (var i = 0; i < quizQ.answers.length; i++) {

    // Create a new button element.
    var button = document.createElement('button');

    // Answer buttons to be in a column style
    button.setAttribute("style", "display: block")

    // Give the button element a unique `qNo` value and the text of the answer.
    button.textContent = qNo + " " + quizQ.answers[i];
    qNo++;

    // Give the button element an event listener that calls the `checkAnswer()` function when the button is clicked.
    button.addEventListener('click', checkAnswer);

    // Append the button element to the `buttonContainer` element.
...
function createNameInput() {
  ...
  nameLabel.textContent = "Enter your initial:";
